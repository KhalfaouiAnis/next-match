...
export async function signInUser(
  ...
  try {
    ...
    if (!existingUser.emailVerified) {
      ...
      return {
        status: "error",
        error: "Please verfy your email address to login.",
      };
    }
...
export async function generateResetPasswordEmail(
  ...
  try {
    ...
    if (!existingUser) return { status: "error", error: "Invalid email" };
    ...
    return {
      status: "success",
      data: "Password reset has been send, please check your email",
    };
  } catch (error) {
    console.log({ error });
    return { status: "error", error: "Something went wrong" };
