...
export default function MessageBox({ message, currentUserId }: Props) {
    ...
    const messageContentClasses = clsx(
        ...
        {
            'roundded-l-xl rounded-tr-xl text-white bg-blue-100': isCurrentUserSender,
            'roundded-r-xl rounded-tl-xl border-gray-200 bg-green-100': !isCurrentUserSender,
        }
    )

    const rendeMessageHeader = () => (
        <div className={clsx('flex items-center w-full', {
            'justify-between': isCurrentUserSender
        })}>
            {message.dateRead && message.recipientId !== currentUserId ? (
                <span className="text-xs text-black text-italic">(seen {timeAgo(message.dateRead)})</span>
            ) : <div />
            }
            <div className="flex">
